...
def get_user_input():
    ...
    ticker = input("Enter the ticker symbol for the report: ").strip()
    ...
    components_input = input("Components: ").strip().split(',')
    
    components = [component.strip() for component in components_input if component.strip() in valid_components]
    if not components:
        print("No valid components selected. Using default components.")
        components = valid_components
    
    return ticker, components

def main():
    """Main function to execute the script."""
    ticker, components = get_user_input()
